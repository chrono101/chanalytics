...
var namesString = "";
for(var md5 in fileNames) {
  namesString = namesString + ("<tr><td>" + generateImageThumb(fileNames[md5]) + "</td><td>" + fileNames[md5].filename + "</td><td>MD5: " + fileNames[md5].md5 + "</td></tr>");
}
var extCountsString = "";
...
$("#imageStats-FnFreqs").html(namesString);
...
function analytic_wordCounts(thread) {
  ...
  var postWordFreq = {}; // { post_number: {term1: count, term2: count}, post_number2:, etc... }
  var postWordCount = {}; // {post_number: number_of_words, post_number2: number_of_words, etc...}
  var wordPostCount = {}; // {word: number_of_posts, word2: number_of_posts, etc...}
  var postTfidf = {}; // {post_number: {term1: tf-idf, term2: tf-idf}, post_number2: {term1: tf-idf}, etc... }
...
$.each(thread.posts, function (i, post) {
  if (post.com) {
    ...
    postWordCount[post.no] = words.length; // How many words are in each post?
    wordFreq = {};
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      wordFreq[word] = (Number(wordFreq[word]) || 0) + 1;
    }
    postWordFreq[post.no] = wordFreq;
  }
});

for (var postNo in postWordFreq) {
  for (var word in postWordFreq[postNo]) {
    wordPostCount[word] = (Number(wordPostCount[word]) || 0) + 1;
  }
}
...
for (var postNo in postWordFreq) {
  ...
  for (var word in postWordFreq[postNo]) {
    var tf = postWordFreq[postNo][word] / postWordCount[postNo];
    var idf = Math.log(thread.posts.length / wordPostCount[word]);
    wordTfidf[word] = (tf * idf);
  
  }
  postTfidf[postNo] = wordTfidf;
}

var tfidfString = "";

for (var postNo in postTfidf) {
  postTfidfString = "<tr><td>Post No: " + postNo + "</td><td>";
  sortedTfidf = []  
  for (var word in postTfidf[postNo]) {    
      sortedTfidf.push([postNo, word, postTfidf[postNo][word]]);    
  }
  sortedTfidf.sort(function (a, b) { return b[2] - a[2]});
  for (var i = 0; i < sortedTfidf.length; i++) {
    postTfidfString = postTfidfString + sortedTfidf[i][1] + " ";
    if (i == 10) break;
  } 
  
  tfidfString = tfidfString + postTfidfString + "</td></tr>";
}

$("#table-tfidf").html(tfidfString);
