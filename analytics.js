    ...
    function getThreadsListing() {
      ...
      $("#thread").empty();
      // Perform JSON request
      // Getting sick of your shit moot
    ...
    function getThreadStats() {
      ...
      jqxhr.done(function (data) {
        ...
        // Image Stats Analytic        
        analytic_images(data);
        // Words Count Analytic
    ...
    function updateStatus(code, time, status) {      
      // Code
      $("#status-http").removeClass();
      if (code == 200) {
        $("#status-http").addClass("text-success").html(code);
      }  else  {
        $("#status-http").addClass("text-danger").html(code);
      }
      // Time
      $("#status-elapsed-time").html(time + "ms");
      // Status Message
      $("#status").html(status);
    }
